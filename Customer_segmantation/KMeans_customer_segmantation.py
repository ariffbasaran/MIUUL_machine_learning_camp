...
######### 1) Genel resmi inceleyelim.

check_data(df)
df = df[~df["Invoice"].str.contains("C", na = False)] #Başında C olanlar iptal olanlardı
df = df[(df["Quantity"] > 0)] # Bizden alışveriş yapmış müşterileri sınıflandırmak istediğimiz için.
df.dropna(inplace=True)
df.describe([0.01, 0.1, 0.25, 0.5, 0.75, 0.90, 0.99])
df[df['Price'] == 0]["StockCode"].unique()
df[df["StockCode"] == 'M'] #Manul isimli işlemin nolduğunu bilemediğim için veriden çıkartıyorum.
df = df[df["StockCode"] != 'M']
invalid_codes = df[df['StockCode'].astype(str).str.contains(r"[a-zA-Z]{3,}")]["StockCode"].unique().tolist()
#Regex ekleyerek stockcode içerisinde 3 ve daha fazla string barındıran stockcodeları liste şeklinde çağırttırıyoruz.
invalid_codes

df.loc[df["StockCode"].isin(invalid_codes)].sample(10) #Rastgele 10 örneğe bakıyoruz. Ağırlıklı olarak postage değerine sahip olanlar geldi.
#Postage'lar muhtemelen kargo ücreti ve bizim işlemimizi etkileyeceği için bunları çıkartacağız.
df[df["StockCode"].isin(invalid_codes)].groupby(["StockCode"]).agg({"Invoice": "nunique",
                                                                       "Quantity": "sum",
                                                                       "Price": "sum",
                                                                       "Customer ID": "nunique"})
#Bunların bir çoğu POST diğerleri de veristenin binde birini bile oluşturmaz o yüzden direkt sileceğiz.

df = df[~df["StockCode"].isin(invalid_codes)].reset_index(drop= True)
check_data(df)
df["TotalPrice"] = df["Quantity"] * df["Price"] #Monetary

#RFM,
# Recency yenilik yani müşterinin en son alışveriş yaptığı tarihle günümüz arasındaki fark,
# freq müşterinin o zamana kadar yapmış olduğu işlem sayısı ,
# monetary müşterinin işlem başına yaptığı harcama

df.describe ([0.01, 0.1, 0.25, 0.5, 0.75, 0.90, 0.99])
max_invoice_date = df["InvoiceDate"].max()
today_date = (max_invoice_date + dt.timedelta (days=2)) #RFM için sabit bir tarih lazım, bu sabit değeri verisetinin yer aldığı maks tarihin 2 gün sonrası.

rfm = df.groupby ("Customer ID").agg ({"InvoiceDate": lambda date: (today_date - date.max ()).days, #recency
                                       "Invoice": "nunique", # freq
...
# LOG TRANSFORMATION
#Daha normal bir dağılım elde etmek için kullanılır.
